...
@Schema(description = "浇注机户内异常上报记录")
@org.hibernate.annotations.Table(
    appliesTo = "jz_hu_nei_exception_reported_record",
    comment = "浇注机户内异常上报记录")
@Table(indexes = {@Index(columnList = "deviceId,uploadDateTime", unique = true)})
public class JzHuNeiExceptionReportedRecord extends Audited implements JzExceptionReportedRecord{
  ...
  private ErpDeviceConfig.JZHuNeiDeviceConfig config;

  @Schema(description = "erp device update data with conformity status,json 字符串")
  @org.hibernate.annotations.Type(type = "json")
  @Column(columnDefinition = "json")
  private JZHuNeiDataUploadDate.HuNeiCheckedExceptionResult uploadDataWithConformityStatus;
}
