...
@Schema(description = "浇注机维克旧异常上报记录")
@org.hibernate.annotations.Table(
    appliesTo = "jz_wei_ke_old_exception_reported_record",
    comment = "浇注机维克旧异常上报记录")
@Table(indexes = {@Index(columnList = "deviceId,uploadDateTime", unique = true)})
public class JzWeiKeOldExceptionReportedRecord extends Audited implements JzExceptionReportedRecord{
  ...
  @Column(columnDefinition = "json") private ErpDeviceConfig.JZWeiKeOldDeviceConfig config;

  @Schema(description = "erp device update data with conformity status,json 字符串")
  @org.hibernate.annotations.Type(type = "json")
  @Column(columnDefinition = "json")
  private JZWeiKeOldDataUploadDate.WeiKeOldCheckedExceptionResult uploadDataWithConformityStatus;
}
